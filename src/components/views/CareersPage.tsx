...
export const CareersPage: React.FC<CareersPageProps> = ({ onBack }) => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0B1929] text-white' : 'bg-[#F8FAFC] text-slate-900'}`}>
      ...
      <header className="sticky top-0 z-40 bg-white/90 dark:bg-[#0B1929]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl 2xl:max-w-[1650px] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <BrandLogo />

          <button
            onClick={onBack}
            className="px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-[#C9A961] hover:text-[#0B1929] text-slate-800 dark:text-slate-200 font-bold text-xs transition-all flex items-center gap-2 cursor-pointer shadow-sm"
          >
      ...
      {/* Hero Banner Area */}
      <div className="relative py-16 sm:py-24 bg-gradient-to-b from-slate-900 to-[#0B1929] text-white overflow-hidden">
      ...
      <main className="max-w-7xl 2xl:max-w-[1650px] mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-8">
        {/* Search & Filter */}
        <div className="bg-white dark:bg-[#0B1929] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl rtl:pr-11 rtl:pl-4 ltr:pl-11 ltr:pr-4 py-3 text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:border-[#C9A961] focus:outline-none"
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredJobs.map((job) => (
            <div
              key={job.id}
              className="bg-white dark:bg-[#0B1929] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm hover:border-[#C9A961] hover:shadow-xl transition-all flex flex-col justify-between group space-y-4"
            >
              ...
              <div className="pt-4">
                <button
                  ...
                  className="w-full py-3.5 px-4 rounded-2xl bg-[#C9A961] hover:bg-[#b3914a] text-[#0B1929] font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
        ...
        <div className="bg-gradient-to-r from-[#0B1929] via-[#0E2035] to-[#0B1929] border border-[#C9A961]/30 rounded-3xl p-8 sm:p-10 text-white shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <button
            ...
            className="px-8 py-4 rounded-2xl bg-[#C9A961] hover:bg-[#b3914a] text-[#0B1929] font-bold text-xs shadow-lg shrink-0 cursor-pointer"
          >
