        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
          {whyUsFeatures.map((item, idx) => {
            ...
            return (
              ...
              >
                <TiltCard
                  className={`h-full rounded-2xl p-8 border transition-all duration-300 flex flex-col justify-between group relative overflow-hidden ${
                    theme === 'dark'
                ...
                >
                  ...
                  <div className="space-y-6 relative z-10">
                    ...
                    <div className="flex items-center justify-between">
                      <div
                        className={`w-14 h-14 rounded-xl border flex items-center justify-center transition-all duration-300 shadow-lg ${
                          theme === 'dark'
                      ...
                      <span className="text-[11px] font-black px-3 py-1 rounded-md bg-[#C9A961]/15 text-[#C9A961] border border-[#C9A961]/30 shadow-sm">
                    ...
                    {/* Title */}
                    <h3
                      className={`text-xl font-black leading-snug transition-colors group-hover:text-[#C9A961] ${
                  ...
                  <div
                    className={`pt-6 mt-6 border-t flex items-center gap-2 text-xs font-black text-[#C9A961] relative z-10 ${
                      theme === 'dark' ? 'border-slate-800' : 'border-slate-100'
                    }`}
                  >
                    <CheckCircle className="w-4 h-4 text-[#C9A961] shrink-0" />
                    <span>{lang === 'ar' ? 'خدمة مضمونة 100%' : '100% Guaranteed Quality'}</span>
                  </div>
                </TiltCard>
              </motion.div>
            );
