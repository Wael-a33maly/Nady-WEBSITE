            ...
            {filteredProjects.map((project, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Top Badge */}
                  <div className="absolute top-3 sm:top-4 rtl:right-3 sm:rtl:right-4 ltr:left-3 sm:ltr:left-4 z-10 flex items-center gap-2">
                    <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full gold-gradient-bg text-[#0B1929] text-[10px] sm:text-xs font-extrabold shadow-md">
                      {lang === 'ar' ? project.badgeAr : project.badgeEn}
                    </span>
                  </div>

                  {/* Bottom Content Area */}
                  <div className="relative z-10 p-4 sm:p-6 md:p-8 space-y-2.5 sm:space-y-3">
                    <div className="flex items-center gap-3 sm:gap-4 text-xs text-[#C9A961] font-medium">
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 shrink-0" />
                        <span className="truncate">{lang === 'ar' ? project.locationAr : project.locationEn}</span>
                      </span>
                      <span className="flex items-center gap-1">
                    ...
                    <h3 className={`text-lg sm:text-xl md:text-2xl font-bold text-white group-hover:text-[#C9A961] transition-colors leading-tight break-words ${
                    ...
                    {/* Stats & CTA */}
                    <div className="pt-3 sm:pt-4 border-t border-slate-700/60 flex items-center justify-between gap-2 sm:gap-4 flex-wrap">
                      <span className="text-[10px] sm:text-xs font-mono font-semibold text-slate-200 bg-[#0B1929]/80 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg border border-slate-700 truncate max-w-[180px] sm:max-w-none">
                        {lang === 'ar' ? project.statsAr : project.statsEn}
                      </span>

                      <div className="flex items-center gap-1.5 text-xs font-bold text-[#C9A961] group-hover:translate-x-1 transition-transform rtl:group-hover:-translate-x-1 shrink-0">
                        <span>{lang === 'ar' ? 'عرض الحالة' : 'View Case'}</span>
                        {lang === 'ar' ? <ArrowLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                      </div>
                    </div>
                  </div>
                </motion.div>
