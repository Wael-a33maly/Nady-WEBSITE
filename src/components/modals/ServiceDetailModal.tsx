        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="space-y-3 pt-2 border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(lang === 'ar' ? selectedService.featuresAr : selectedService.featuresEn).map((feat, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-slate-50 dark:bg-[#112236] border border-slate-200 dark:border-slate-800 flex items-center gap-2.5 text-xs text-slate-800 dark:text-slate-200">
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <button
              type="button"
              onClick={() => setSelectedService(null)}
              className="w-full sm:w-auto px-6 py-3 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl gold-gradient-bg text-[#0B1929] font-bold text-sm flex items-center justify-center gap-2 cursor-pointer shadow-xl hover:brightness-110"
            >
